...
import {
  Box, Grid, Paper, Typography, IconButton, AppBar, Toolbar,
  List, ListItemText, ListItemButton, ListItem, TextField,
  Slider, Chip, FormControl, InputLabel, Select, MenuItem
} from '@mui/material';
...
const CaseDetails = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      handleError(err, 'Failed to load case data');
  ...
  const saveCase = async (updatedCase: Partial<Case>) => {
    ...
    try {
      ...
      handleError(err, 'Failed to save case');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleGenerateGradCAM = async (diagnosis: Diagnosis) => {
    if (!diagnosis.name || !diagnosis.gradcamData) return;
    ...
    try {
      ...
      handleError(err, 'Failed to load Grad-CAM');
    } finally {
  ...
  const createCanvasFromBase64 = (base64Data: string): Promise<HTMLCanvasElement> => {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => reject("Failed to load image from base64 data");
      img.src = base64Data;
    });
  };
  ...
  const DiagnosisTab = () => (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {showAddDiagnosis && pendingBoundingBox && (
        <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
          <Box sx={{ p: 2, border: '1px solid', borderColor: 'primary.main', borderRadius: 1, bgcolor: 'rgba(63, 136, 242, 0.02)' }}>
                ...
                >
                  <Box sx={{ width: '100%' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                      <Typography variant="body2" fontWeight={selectedBoundingBox === diagnosis.boundingBox ? 600 : 400} noWrap>
                        {diagnosis.name}
                      </Typography>
                      <IconButton 
                        size="small" 
  ...
  const NotesTab = () => (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <List sx={{ p: 0 }}>
            {currentCase.notes.map((note) => (
              <ListItem key={note.id} disablePadding sx={{ mb: 0.5 }}>
                <ListItemButton sx={{ borderRadius: 1, display: 'block' }}>
                  <Box sx={{ width: '100%' }}>
                    {editingNote?.id === note.id ? (
                      ...
                      <Box>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                          <Typography variant="body2" sx={{ lineHeight: 1.4, flex: 1, mr: 1 }}>
                            {note.text}
                          </Typography>
      ...
      {/* Add Note Button - Only show when not adding */}
      {!showAddNote && (
  ...
  if (!currentCase) return <Error message="Case not found" onRetry={() => navigate(`/patients/${patientId}`)} />;

  const tabs: TabItem[] = [
    { label: 'Diagnosis', content: <DiagnosisTab /> },
        ...
        <Grid container sx={{ height: '100%' }} spacing={2}>
          ...
          <Grid item md={6} sx={{ height: '100%' }}>
            <Paper sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <Box sx={{ flex: 1, position: 'relative', bgcolor: 'black', borderRadius: '0 0 12px 12px', overflow: 'hidden' }}>
                ...
                {gradcamMode === 'normal' ? (
                  <XRayCanvas
                    ...
                    onBoundingBoxSelected={setSelectedBoundingBox}
